...
public class Player : MonoBehaviour
{
    ...
    [SerializeField] HighscoreTable scoreSystem;
    ...
    public void FixedUpdate()
    {
        ...
        if (isDead)
        {
            try
            {
                ...
                if (distance > float.Parse(scoreSystem.Load()))
                {
                    scoreSystem.Save(distance.ToString());
                }
            }
            catch
            {
        ...
        {
        FindObjectOfType<AudioManager>().AudioTrigger(AudioManager.SoundFXCat.Enemy, transform.position, 1f);
